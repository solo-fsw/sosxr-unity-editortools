...
public class SetIconWindow : EditorWindow
{
    ...
    private const int ICON_HEIGHT = 256;
    private const int BUTTON_WIDTH = 256;
    private const int ICONS_PER_ROW = 5; // Icons displayed per row
    ...
    {
        if (_selectedIconIndex < ICONS_PER_ROW)
        {
            return;
        }

        _selectedIconIndex -= ICONS_PER_ROW;
        Event.current.Use();
    }
    ...
    {
        if (_selectedIconIndex + ICONS_PER_ROW >= _iconTextures.Count)
        {
            return;
        }
