...
namespace EventVisualizer.Base
{
    public class EventsGraphWindow : EditorWindow
    {
        ...
        {
            var editor = GetWindow<EventsGraphWindow>();
        ...
        private void OnGUI()
        {
            ...
            if (_graphGUI != null)
            {
                var r = EditorZoomArea.Begin(_zoom, _zoomArea);
                // Main graph area
            ...
            var result = GUILayout.Toolbar(-1, toolbarStrings);
        ...
        private void OnSceneGUI(SceneView sceneView)
        {
            ...
            {
                var entity = elem.Entity as GameObject;

                if (null != entity)
                {
                    var isEntitySelected = CheckSelection(entity.gameObject);
                    bool behindScreen;
                    ...
                    if (isEntitySelected)
                    {
                        ...
                        for (var i = 0; i < elem.Inputs.Count; i++)
                        {
                            var ev = elem.Inputs[i];
                            var receiver = ev.receiver as GameObject;
                            var sender = ev.sender as GameObject;

                            if (null != sender && receiver != ev.sender)
                            {
                                if (behindScreen)
                                {
                                    senderPos2D = WorldToGUIPoint(entity.transform.position, sender.transform.position, out behindScreen);
                                }
                        ...
                        for (var i = 0; i < elem.Outputs.Count; i++)
                        {
                            ...
                            if (null != receiver && receiver != ev.sender)
                            {
                                if (behindScreen)
                                {
                                    senderPos2D = WorldToGUIPoint(entity.transform.position, receiver.transform.position, out behindScreen);
                                }
                    ...
                    else
                    {
                        for (var i = 0; i < elem.Inputs.Count; i++)
                        {
                            ...
                            if (null != sender)
                            {
                                var isSenderSelected = CheckSelection(sender.gameObject);
                        ...
                        for (var i = 0; i < elem.Outputs.Count; i++)
                        {
                            ...
                            if (null != receiver)
                            {
                                var isReceiverSelected = CheckSelection(receiver.gameObject);
        ...
        private static void DrawConnection(EventCall ev, Bezier b)
        {
            ...
            {
                var pos = EdgeGUI.CalculateBezierPoint(trigger, p1, p3, p4, p2);
        ...
        private static Vector2 WorldToGUIPoint(Vector3 p, Vector3 p2, out bool behindScreen)
        {
            ...
            var viewPos = cam.WorldToViewportPoint(p);
            ...
            if (behindScreen)
            {
                ...
                var proj = r.origin + r.direction * enter;
